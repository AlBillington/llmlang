...
- **Commentary bullet** — one plain-English sentence, `# ` prefixed, valid anywhere a behavior bullet is valid (top-level under an entry, or nested, including under a decision/case/repeat). For a property, guarantee, rationale, or scope note about the entry as a whole — something true independent of any specific call, with no condition to lead and no step to perform (`# reproduces the same hashes when run again against an unchanged codebase`). Not a weaker behavior bullet: a fact that's part of executing the entry, even a conditional one, is still a behavior bullet (§4c). Hashed and reviewed exactly like a behavior bullet - the only place it's treated differently is flow generation (§4f), which excludes it the same way it excludes a `~` test bullet, since neither describes execution.

  No fixed position: place it as close as possible to what it's about, not always first. A note qualifying one specific bullet sits directly next to that bullet; a batch of genuinely entry-wide properties can group together wherever they read best (often near the end, after the main behavior is established). The one placement that's invalid: between a decision/case's own condition and its `if yes`/`if no`/`for X case:` outcomes - that relationship has to read as one contiguous unit, so a commentary bullet about it goes before the whole block or after, never spliced into the middle.
...
A sibling `<stem>.llmflow` file is a generated, human-readable execution map, composed by walking every `@entry-point` entry and inlining each `→ call` target's own bullets, recursively — the same bullets already described here, rendered as one flattened, order-following trace instead of scattered across the `.llm` tree's folder/file/class structure. A rendered `→ call` line carries the target's own parenthesized summary alongside its full canonical name, so a reader can follow the trace without jumping to the target's own header first. `~` test bullets and `#` commentary bullets are both excluded from the rendering - neither describes execution. It is never hand-edited; see [flow-format.md](flow-format.md) for how it's generated and what a mismatch means.
