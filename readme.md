...
A big part of the point is accountability. An LLM working through a codebase can forget a step, skip an entry, or let the code quietly drift from what the spec says, the same way a person can. The tooling in §3 exists to make that impossible to do silently. Every named entry's code is hash-tracked against its spec, so a mismatch always surfaces as something to look at. `finalize` (§3.4) refuses to write a new lockfile unless every currently flagged entry has an explicit, reviewed disposition, so an LLM working through a batch of fixes can't leave one unaddressed without it being visible in the diff. `--coverage` (§7) extends the same enforcement to the code side: every function needs a comment handle or an explicit exemption, so nothing gets written that isn't accounted for somewhere. None of this tells an LLM what to write. It makes sure whatever gets written is checked, and that the checking can't be skipped.
...
**The compiler isn't software to build, it's an LLM, prompted correctly.** Given a dirty entry's bullets and the surrounding context, an LLM already writes the code; that's exactly what happened by hand, in conversation, for every example in this repo. The tooling verifies what an LLM produces; it doesn't invoke one itself. See §6.
