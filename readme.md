...
`build.py <file>.llm --finalize` closes that gap. It reads `<stem>.llmchanges.json` — a real, git-tracked sibling file next to the `.llm` and `.llmlock`, `{tracking_key: "free text describing what changed, or why nothing needed to"}` — one entry per currently flagged key. This file is authored the same way the `.llm` file itself is (by hand or by an LLM, reviewed via its own git diff) and is never written by the tool; `--finalize` only reads it, exactly the same read-only relationship `--check` already has to the `.llm` file. Before writing anything, finalize re-runs the same check `--check` does and refuses outright, listing exactly which keys are missing, unless every currently flagged named entry has a non-empty disposition in the changes file. There's no fixed vocabulary for the disposition text (same "not a fixed vocabulary" stance as Hints) — the only mechanically enforced thing is *presence*, not content.
...
Each entry's disposition, once accepted, is stored on it (`LockEntry`'s `last_change` field) bound to the exact `text_hash`/`code_hash` it was recorded against, and carried forward unchanged on later rebuilds only while both hashes still match — an entry that wasn't touched in a given round doesn't need to be re-justified in that round's changes file, but one that drifted through an *unguarded* rebuild instead of `--finalize` loses its now-stale disposition rather than keeping a claim that no longer describes it.
